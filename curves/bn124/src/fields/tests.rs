...
use ark_ff::{
    biginteger::{BigInt, BigInteger, BigInteger128},
...
fn test_fq_repr_from() {
    assert_eq!(BigInteger128::from(100u64), BigInt::new([100, 0]));
}

#[test]
fn test_fq_repr_is_odd() {
    assert!(!BigInteger128::from(0u64).is_odd());
    assert!(BigInteger128::from(0u64).is_even());
    assert!(BigInteger128::from(1u64).is_odd());
    assert!(!BigInteger128::from(1u64).is_even());
    assert!(!BigInteger128::from(324834872u64).is_odd());
    assert!(BigInteger128::from(324834872u64).is_even());
    assert!(BigInteger128::from(324834873u64).is_odd());
    assert!(!BigInteger128::from(324834873u64).is_even());
}

#[test]
fn test_fq_repr_is_zero() {
    assert!(BigInteger128::from(0u64).is_zero());
    assert!(!BigInteger128::from(1u64).is_zero());
    assert!(!BigInt::new([0, 1]).is_zero());
}

#[test]
fn test_fq_repr_num_bits() {
    let mut a = BigInteger128::from(0u64);
    assert_eq!(0, a.num_bits());
    a = BigInteger128::from(1u64);
    for i in 1..129 {
...
#[test]
fn test_fq_num_bits() {
    assert_eq!(Fq::MODULUS_BIT_SIZE, 124);
}
...
#[test]
fn test_fq_ordering() {
    // BigInteger128's ordering is well-tested, but we still need to make sure the
    // Fq elements aren't being compared in Montgomery form.
    for i in 0..100u64 {
        assert!(Fq::from(BigInteger128::from(i + 1)) > Fq::from(BigInteger128::from(i)));
    }
}
...
fn test_fq_legendre() {
    ...
    assert_eq!(
        QuadraticResidue,
        Fq::from(BigInteger128::from(4u64)).legendre()
    );
    assert_eq!(
        QuadraticNonResidue,
        Fq::from(BigInteger128::from(3u64)).legendre()
    );
}
